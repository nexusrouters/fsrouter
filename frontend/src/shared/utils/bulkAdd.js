...
// Background: backend upserts apikey connections BY NAME (connectionsRepo).
// A colliding name overwrites an existing key instead of inserting a new one.
// Bulk-add used to derive "<base> <lineIndex>" from paste position, blind to
// existing names, so re-adding keys silently replaced earlier ones.
// This planner gap-fills the smallest free "<base> <n>" against both existing
// connection names and names already assigned earlier in the same batch.
