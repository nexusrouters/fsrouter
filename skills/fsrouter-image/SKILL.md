---
name: fsrouter-image
description: Generate images via FSRouter /v1/images/generations using OpenAI / Gemini Imagen / DALL-E / FLUX / MiniMax / SDWebUI / ComfyUI / Codex models. Use when the user wants to create, generate, draw, or render an image, picture, or text-to-image (txt2img).
---

# FSRouter — Image Generation
...
```bash
curl $FSROUTER_URL/v1/models/image | jq '.data[].id'
# Per-model params/options (size enum, quality enum, capabilities like edit)
curl "$FSROUTER_URL/v1/models/info?id=openai/dall-e-3"
```

## Endpoint

`POST $FSROUTER_URL/v1/images/generations`
...
```bash
curl -X POST "$FSROUTER_URL/v1/images/generations?response_format=binary" \
...
```js
const r = await fetch(`${process.env.FSROUTER_URL}/v1/images/generations`, {
  method: "POST",
