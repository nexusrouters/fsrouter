...
function systemBlocksFromOpenAI(messages) {
  ...
  for (const message of messages) {
    ...
    const text = textBlocksFromContent(message.content).map((block) => typeof block.text === "string" ? block.text : "").filter(Boolean).join("\n");
...
function toolResultContentFromMessage(message) {
  ...
  if (Array.isArray(content)) {
    ...
    for (const part of content) {
      ...
      else if (p.type === "json" && p.json !== void 0) result.push({ json: p.json });
      else if (p.content !== void 0) result.push({ text: toText(p.content) });
    }
...
function isToolResultOnlyMessage(message) {
  return message?.role === "user" && Array.isArray(message.content) && message.content.length > 0 && message.content.every((block) => Boolean(getToolResultIdFromBlock(block)));
}
...
function sanitizeBedrockToolPairs(messages) {
  ...
  const validResultCounts = /* @__PURE__ */ new Map();
  for (let i = 0; i < normalized.length; i++) {
    const message = normalized[i];
    if (message?.role !== "assistant" || !Array.isArray(message.content)) continue;
    const nextMessage = normalized[i + 1];
    const nextResultIds = new Set(
      nextMessage?.role === "user" && Array.isArray(nextMessage.content) ? nextMessage.content.map(getToolResultIdFromBlock).filter(Boolean) : []
    );
...
function toolConfigFromOpenAI(tools, toolChoice) {
  ...
  if (bedrockTools.length === 0) return void 0;
...
function openAIToBedrockConverse(model, body) {
...
function openAIChunk(model, delta, finishReason = null, usage = void 0) {
  const chunk = {
...
function createOpenAIStreamFromBedrock(stream, model) {
  const blockToolIndexes = /* @__PURE__ */ new Map();
  let nextToolIndex = 0;
  ...
  return new ReadableStream({
    async start(controller) {
      try {
        ...
        controller.enqueue(sse(openAIChunk(model, {}, finishReason, finalUsage || void 0)));
...
class BedrockExecutor extends BaseExecutor {
  ...
  async execute({ model, body, stream, credentials, signal, log }) {
    ...
    try {
      ...
      if (stream) {
        const output2 = await client.send(new ConverseStreamCommand(transformedBody), {
          abortSignal: signal || void 0
        });
        return {
          response: new Response(createOpenAIStreamFromBedrock(output2.stream, model), {
            status: 200,
...
var bedrock_default = BedrockExecutor;
export {
  BedrockExecutor,
  bedrock_default as default,
  openAIToBedrockConverse
};
