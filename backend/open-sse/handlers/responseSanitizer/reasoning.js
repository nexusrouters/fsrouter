...
export function collapseExcessiveNewlines(text) {
  return text.replace(EXCESSIVE_NEWLINES, "\n\n");
}

export function cleanReasoningFragment(text) {
  return text.replace(REASONING_TAG_FRAGMENT_REGEX, "").trim();
}

export function splitClosingOnlyReasoningPrefix(text) {
 ...
 */
export function extractThinkingFromContent(text) {
