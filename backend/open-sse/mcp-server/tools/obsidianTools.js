import { z } from "zod";
import { createObsidianClient, createSyncServerClient, getSyncToken } from '../../../lib/obsidian/api.ts.js';
import {
  getObsidianConfigForApiKey
} from '../../../lib/db/obsidian.ts.js';
...
function requireToken(apiKeyId) {
  const config = getObsidianConfigForApiKey(apiKeyId);
  if (!config.token) {
    throw new Error(
      "Obsidian API token not configured. Set it in Settings > Context Sources" + (apiKeyId ? " or in your API key's context source settings" : "") + "."
    );
  }
  return config.token;
}
function getClient(apiKeyId) {
  const config = getObsidianConfigForApiKey(apiKeyId);
