...
async function handleSimulateRoute(args) {
  ...
  try {
    ...
    const quota = quotaRaw.status === "fulfilled" ? normalizeQuotaResponse(quotaRaw.value) : normalizeQuotaResponse({});
    const targetCombo = args.combo ? combos.find(
      (combo) => toString(combo.id) === args.combo || toString(combo.name) === args.combo
    ) : combos.find((combo) => combo.enabled !== false);
    if (!targetCombo) {
      return {
        content: [
          { type: "text", text: JSON.stringify({ error: "No matching combo found" }) }
        ],
        isError: true
      };
    }
...
async function handleSetRoutingStrategy(args) {
  ...
  try {
    ...
    const combo = combos.find(
      (comboEntry) => toString(comboEntry.id) === args.comboId || toString(comboEntry.name) === args.comboId
    );
    if (!combo) {
...
async function handleTestCombo(args) {
  ...
  try {
    ...
    const providerResults = results.map(
...
async function handleGetProviderMetrics(args) {
  ...
  try {
    ...
    const health = healthRaw.status === "fulfilled" ? toRecord(healthRaw.value) : {};
    const quota = quotaRaw.status === "fulfilled" ? normalizeQuotaResponse(quotaRaw.value, { provider: args.provider }) : normalizeQuotaResponse({});
    ...
    const result = {
      ...
      successRate: toNumber(analytics.successRate, 1),
...
async function handleBestComboForTask(args) {
  ...
  try {
    ...
    if (enabledCombos.length === 0) {
      return {
        content: [
          { type: "text", text: JSON.stringify({ error: "No enabled combos available" }) }
        ],
        isError: true
      };
    }
    const scored = enabledCombos.map((combo) => {
      ...
      const isFree = name.includes("free") || models.every((model) => model.provider.toLowerCase().includes("free"));
    ...
    const alternatives = scored.slice(1, 4).map((s) => ({
      id: s.combo.id,
      name: s.combo.name,
      tradeoff: s.isFree ? "free but may have limits" : s.score < best.score * 0.5 ? "cheaper but slower" : "similar quality, different providers"
    }));
...
async function handleDbHealthCheck(args) {
  const start = Date.now();
  const autoRepair = args.autoRepair === true;
  try {
    const { runManagedDbHealthCheck } = await import('../../../lib/db/core.ts.js');
...
async function handleCacheFlush(args) {
  const start = Date.now();
  try {
...
async function handleOneproxyFetch(args = {}) {
  const start = Date.now();
  try {
    const params = new URLSearchParams();
...
async function handleOneproxyRotate(args = {}) {
  const start = Date.now();
  try {
    const body = {};
...
async function handleOneproxyStats(args = {}) {
  const start = Date.now();
  try {
    ...
    const status = {
      ...
      lastSyncError: typeof statusRaw.last_sync_error === "string" ? statusRaw.last_sync_error : null,
