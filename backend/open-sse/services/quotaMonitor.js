...
import { getSessionInfo } from "../utils/sessionManager.js";
...
const alertSuppression = new Map();
...
export function registerMonitorFetcher(provider, fetcher) {
  quotaFetcherRegistry.set(provider, fetcher);
  registerQuotaFetcher(provider, fetcher);
}

export function isQuotaMonitorEnabled(connection) {
...
export function startQuotaMonitor(sessionId, provider, accountId, connection) {
...
export function stopQuotaMonitor(sessionId) {
  const state = activeMonitors.get(sessionId);
  if (!state) return;
  state.stopped = true;
  if (state.timer) {
...
export function getQuotaMonitorSnapshot(sessionId) {
  const state = activeMonitors.get(sessionId);
  if (!state || state.stopped) return null;
  return toPublicSnapshot(sessionId, state);
}

export function getQuotaMonitorSnapshots() {
  const snapshots = [];
...
export function getQuotaMonitorSummary() {
...
export function clearQuotaMonitors() {
