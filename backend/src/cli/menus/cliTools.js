...
async function buildCodexHeader() {
  ...
  const { installed, hasFSRouter, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, hasFSRouter, settings } = result.data;
  ...
  // Extract FSRouter custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:FSRouter-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract FSRouter provider config
  const provider = settings?.models?.providers?.["fsrouter"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("fsrouter/") ? primary.replace("fsrouter/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasFSRouter, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use FSRouter\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
