...
function buildMenuItems(port, autostartEnabled) {
  return [
    { title: `FSRouter (Port ${port})`, tooltip: "Server is running", enabled: false },
  ...
  try {
    ...
    if (isV2) {
      ...
      trayInstance.ready().catch((err) => {
        process.stderr.write(`[fsrouter] tray failed to start: ${err && err.message ? err.message : err}\n`);
      });
    } else {
    ...
    process.stderr.write(`[fsrouter] tray init error: ${err.message}\n`);
    return null;
  }
}
